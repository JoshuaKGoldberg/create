	...
	describe("take", () => {
		...
		const blockUsingTake = schema.createBlock({
			produce({ take }) {
				return {
					files: {
						"value.txt": take(inputString, { value: "abc" }),
					},
				};
			},
		});

		const presetUsingTake = schema.createPreset({
			blocks: [blockUsingTake()],
		});

		it("uses the native take when a take is not provided", async () => {
			const actual = await testPreset(presetUsingTake, {
				options: { value: "def" },
			});
