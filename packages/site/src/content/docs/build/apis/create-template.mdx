...
A synchronous function that receives a [Template Context](/build/details/contexts) and returns a [`Creation`](/build/concepts/creations).
...
These will show up as placeholder values in the `create` CLI if the property isn't provided explicitly or inferred by [`prepare()`](#prepare).
...
See [create#190 🚀 Feature: More succinct option defaults with Inputs](https://github.com/JoshuaKGoldberg/create/issues/190).
:::
