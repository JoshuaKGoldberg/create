...
This can be useful if you'd like a production's [Inputs](/build/concepts/inputs) to run in a virtual environment or otherwise augment system interactions.
...
For example, the [Stratum engine's Bases](/engines/stratum/concepts/bases) can be produced with `prepareOptions`:
