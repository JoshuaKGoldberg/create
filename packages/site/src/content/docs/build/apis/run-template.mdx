...
Given a [Template](/build/concepts/templates) and any settings, creates and applies [Creation](/build/concepts/creations) output by running the template's production function(s).

`produceTemplate` takes in up to two arguments:

1. A template
2. A settings object, only required if the template includes required options.
   It may contain properties from a [Template Context](/build/details/contexts#template-context):
...
```ts
import { createTemplate, runTemplate } from "create";
import { z } from "zod";
...
Use the asynchronous [`produceOptions`](/build/apis/produce-options) API to generate options that can be passed as [settings `options`](#options).
:::
