...
_Context_ objects are provided to producer methods of [Inputs](/build/concepts/templates) and [Templates](/build/concepts/templates).
Each contains shared helper functions and information.

- Inputs APIs receive an [Input Context](#input-context)
- Options APIs receive an [Options Context](#options-context)
- Template APIs receive an [Template Context](#template-context)

## Input Context

This context object is provided to [`produceInput`](/build/apis/produce-input).
...
export const inputGitUserEmail = createInput({
	async produce({ fetchers, offline }) {
		return offline
			? undefined
			: await fetchers.fetch(args.resource, args.options);
	},
});
```
...
- [`produceOptions`](/build/apis/produce-options)
...
[Template `prepare()` methods](/build/apis/create-template#prepare) are designed to fill in any options not manually provided by the user.
