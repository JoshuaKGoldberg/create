...
A _Creation_ is what's returned by a template's `produce()` method.
...
- ["Indirect" creations](#indirect-creations) to hint to the `create` CLI:
...
For example, this template an executable `.husky/pre-commit` file:
...
See [Packages > `create-fs`](/build/packages/create-fs) for more information on the `files` format.
...
These Creation properties produce information purely for the `create` CLI, not for the new repository.
