...
Running `create` on the CLI will always infer one of the modes.
...
As the production is run, the `create` engine will add in additional creations from the template's [`setup()` method](/build/apis/create-template#transition), if it exists.
Those creations typically includes example starter files or one-time API registrations.

When running `create` on the CLI in setup mode, it will:
...
As the production is run, the `create` engine will add in additional creations from the template's [`transition()` method](/build/apis/create-template#transition), if it exists.
Those creations typically includes scripts that delete files from known no-longer-used pieces of tooling.

When running `create` on the CLI in transition mode, it will:
