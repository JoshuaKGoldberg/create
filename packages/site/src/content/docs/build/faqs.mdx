...
Having the `create` CLI map from complex conditional Zod types to CLI flags is a difficult task.
Soon, `create`-specific tooling will be able to describe full documentation websites that will also need to parse CLI flags.
...
If you have a use case for root-level flags that shouldn't be made into an options object property, please [file a feature request issue on `create`](https://github.com/JoshuaKGoldberg/create/issues/new?template=03-feature.yml).
...
See the [`create-example` Base `prepare()` implementation](https://github.com/JoshuaKGoldberg/create-example/blob/c6714eeb02a5106eb5e4e4694f1e8b590444030d/src/base.ts#L138) (TODO: FIX LINK) for an example of values used across Options.

### Why is the template `prepare()` method synchronous?
...
[`create-testers`](/build/packages/create-testers) functions allow injecting mock versions of [context properties](/build/details/contexts).
...
### Why is the `produceOptions()` API not a part of `produceTemplate()` or `runTemplate()`?
...
If you have a use case for information that should be asynchronously loaded but cannot be an option, please [file a feature request issue on `create`](https://github.com/JoshuaKGoldberg/create/issues/new?template=03-feature.yml).
