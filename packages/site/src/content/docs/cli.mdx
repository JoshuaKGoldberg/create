...
The `create` CLI will automatically detect which [`--mode`](#--mode) it is being run in:
...
The first argument passed to `create` can be a shorthand [`--from`](#--from) for an npm package default-exporting a template.
