...
Stratum generally resuses existing Bingo runner APIs when possible:
...
Any of [Addons](/engines/stratum/apis/creators#createblock-addons) to pass to the Preset's Blocks.
...
Although Presets are associated with [Bases](/engines/stratum/concepts/bases), running `runPreset` does not automatically run [`prepareBase`](#preparebase).
