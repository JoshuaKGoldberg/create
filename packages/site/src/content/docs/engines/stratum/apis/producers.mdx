...
Stratum generally resuses existing Bingo producer APIs when possible:
...
Given a [Block](/engines/stratum/concepts/blocks), creates a [Creation](/build/concepts/creations) output by running its [`produce()`](/engines/stratum/apis/creators#createblock-produce).
...
Any number of [Addons](/engines/stratum/apis/creators/#createblock-addons) defined by the Block.
...
2. `context` _(optional)_: any properties from a [Preset Context](/engines/stratum/details/contexts#preset-contexts)
...
Any [Addons](/engines/stratum/apis/creators#createblock-addons) to pass to the Preset's Blocks.
...
Although Presets are associated with [Bases](/engines/stratum/concepts/bases), running `producePreset` does not automatically run [`prepareBase`](#preparebase).
If you want to include the inferred options from a Preset's Base you'll have to call the `prepareBase` API first yourself.
