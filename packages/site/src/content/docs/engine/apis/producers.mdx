...
For example, given this Preset that includes the block from [`produceBlock`](#produceblock), `produceTemplate` can run its `produce()` with any provided options:
...
await produceTemplate(preset, { options: { title: "My App" } });
```

### Adding Base Options

Although presets are associated with [Bases](../concepts/bases), running `produceTemplate` does not automatically run [`produceBase`](#producebase).
...
// { creation: ... }
await produceTemplate(preset, { options });
```
...
```ts
import { Preset, produceTemplate } from "create";
import { z } from "zod";
