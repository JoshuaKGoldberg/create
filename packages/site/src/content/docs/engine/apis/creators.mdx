...
- [`createBase`](#createbase): creates a new [Base](../concepts/bases)
  ...
  - [`createTemplate`](#createtemplate): creates a new [Template](../concepts/templates) for the Base
...
Options defined in a Base may use the Zod `.default()` property to indicate suggested defaults.
These will show up as placeholder values in the `create` CLI if the property isn't provided explicitly or by [`produce`](#createbase-produce).

For example, this Base defines a required `description` string that suggests a happy text message:
...
Bases on their own do not have to have any particular named Options.
However, in order to be used with `--mode initialize`, two Options must be present:
...
For example, a Base that defines both:
...
This is so that `--mode initialize` can create a new repository on GitHub using those two Options as a repository locator.
...
[Templates](../concepts/templates) can be created by the `createTemplate()` method of a [Base](../concepts/bases).
...
- `suggested` _(optional)_: Which Preset should be selected by default in CLIs
