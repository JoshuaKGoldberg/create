(() => {
		...
		l = class {
			...
			initContainer(e) {
				...
				if (t.tagName === "INPUT") {
					...
					t.innerHTML = "";
					let s = 0;
					for (; document.querySelector(`#pfmod-input-${s}`); ) {
						s += 1;
					}
					let n = new r("form")
	...
	var g = (i) => {
			...
			if (Array.isArray(i) && i.every((e) => e instanceof Element)) {
				return i;
			}
    ...
    <div class="pagefind-modular-list-inner">
		...
		d = class {
			...
			waitForIntersection() {
				if (!this.placeholderNodes?.length) {
					return;
				}
				let e = {
