...
describe("parseTransitionSource", () => {
	it("returns a CLI error when configFile and from are undefined", () => {
		...
		expect(actual).toEqual(
			new Error(
				"Existing repository detected. To transition an existing repository, either create a create.config file or provide the name or path of a template.",
			),
		);
	});
	...
	it("returns a CLI error when configFile and from are both defined", () => {
		...
		expect(actual).toEqual(
			new Error(
				"--mode transition cannot combine an existing config file (create.config.js) with an explicit --from (my-app).",
			),
		);
	});
