...
export function getRepositoryLocator(options: Record<string, unknown>) {
	if (!isStringLike(options.owner)) {
		throw new Error(
			`To run with --mode initialize, --owner must be a string-like, not ${typeof options.owner}.`,
		);
	}

	if (!isStringLike(options.repository)) {
		throw new Error(
			`To run with --mode initialize, --repository must be a string-like, not ${typeof options.repository}.`,
		);
	}
